...
KobukiBase::KobukiBase(const char* filename):
  ...
  m_WheelDrop(0),
  m_Cliff(0),
  m_LeftEncoder(0), m_RightEncoder(0),
  m_LeftPWM(0), m_RightPWM(0),
  m_Button(0),
  m_Charger(0),
  m_Battery(0),
  m_OverCurrentFlag(0),
  m_InertialAngle(0), m_InertialAngleRate(0),
  m_BaseControlSpeed(0), m_BaseControlRadius(0),
  m_DigitalOut(0), m_ExtPower(15),
  m_LED(0)
{
...
void KobukiBase::baseControl(const uint16_t speed_mm_per_sec,
			 const uint16_t radius_mm) {
  m_BaseControlSpeed = speed_mm_per_sec;
  m_BaseControlRadius = radius_mm;
}

void KobukiBase::privateBaseControl(const uint16_t speed_mm_per_sec,
			 const uint16_t radius_mm) {
  Packet packet(6);
  packet[0] = 1;
  packet[1] = 4;
